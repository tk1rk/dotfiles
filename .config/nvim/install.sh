...
ZSH_SHELL_CONFIG=$ZDOTDIR/.zshrc
...
if [[ "$OSTYPE" = "darwin"* ]]; then
   ...
   sudo pacman -Syy neovim python python-pip python-pipenv git curl autoconf automake libtool ctags 
fi
...
if [[ "$OSTYPE" = "darwin"* ]]; then
    brew install the_silver_searcher ripgrep bat bat-extras 
else
    sudo pacman -Syy ripgrep fd bat bat-extras the_silver_searcher
fi
